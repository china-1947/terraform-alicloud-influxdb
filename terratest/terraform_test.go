...
func TestTerraformBasicExampleNew(t *testing.T) {
	...
	instanceAlias := fmt.Sprintf("tf-test-%d", random.Random(100, 1000))
	...
	thisInstanceAlias := terraform.Output(t, terraformOptions, "this_instance_alias")

	// Verify we're getting back the outputs we expect
	assert.Equal(t, thisInstanceAlias, instanceAlias)
}
